...
public class TestLoginEvt extends WindowAdapter implements ActionListener  {
	...
	public String login( String id, String pass) throws SQLException{
		...
		//4. 쿼리문 수행후 결과 얻기
			...
			.append("where id= '").append(dbo_id).append("' and ")
			.append(" pass= '").append(dbo_pass).append("'");
	...
	public void actionPerformed(ActionEvent ae) {
		if(ae.getSource()==tl.getJtfId() || ae.getSource()==tl.getJpfPass() || ae.getSource()==tl.getJbtLogin()) {
			String id=tl.getJtfId().getText().trim();
			String pass=new String(tl.getJpfPass().getPassword()).trim(); //getpassword의 반황형이 char라서 new string으로 해준것.
